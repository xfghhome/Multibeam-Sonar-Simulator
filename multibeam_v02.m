...
% applies 128‑channel Delay‑and‑Sum beamforming, and draws an
% upward‑facing polar‑style intensity map (扇形)。
% -------------------------------------------------------------
...
structMeshPreparation.vertexScaling          = 1/1000;  % mm→m
...
aCoordsReceivers = [zeros(N_elem,1) , yCoords.' , zeros(N_elem,1)];

structSensor = struct();
structSensor.position        = [2 0 0].';    % array reference (m)
structSensor.orientation     = [0 0 0];
structSensor.coordsEmitter   = [1.98 0 0];    % emitter 2 cm in front
structSensor.coordsReceivers = aCoordsReceivers;
structSensor.nMics           = N_elem;

%% ---------- Simulation parameters ----------
structSimulationParameters = struct();
structSimulationParameters.doPlot           = 0;
structSimulationParameters.numSamplesImpresp= round(0.03*fs); % 30 ms
